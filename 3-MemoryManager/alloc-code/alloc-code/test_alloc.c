...
int main()
{
	...
	*/
	...
	if (strBig == NULL)
	{
    	printf("Test 6 passed: Allocation larger than page size failed\n");
	}
	else
	{
    	printf("Test 6 failed: Allocation larger than page size succeeded unexpectedly\n");
	}
	...
	if (cleanup() == 0)
	{
	    printf("Test 7: Cleanup successful\n");

	    // Attempt to allocate after cleanup
	    char *strAfterCleanup = alloc(128);
	    if (strAfterCleanup == NULL)
	    {
	        printf("Test 7 passed: Allocation failed after cleanup\n");
	    }
	    else
	    {
	        printf("Test 7 failed: Allocation succeeded after cleanup\n");
	    }
	}
	else
	{
	    printf("Test 7 failed: Cleanup failed\n");
	}
